...
public sealed class ManufacturingBehavior : ICreatureBehavior
{
    public void Execute(CreatureBehaviorContext context, GameTime time)
    {
        ...
        if (needs is not null)
        {
            ...
            needs.Tiredness = Math.Min(1f, needs.Tiredness + 0.0015f * time.DeltaSeconds);
        }

        var xp = context.Entity.TryGetComponent<ExperienceComponent>();
        if (xp is not null)
        {
            xp.CurrentExperience += 2;
