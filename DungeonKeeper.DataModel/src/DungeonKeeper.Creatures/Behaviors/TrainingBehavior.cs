...
public sealed class TrainingBehavior : ICreatureBehavior
{
    public void Execute(CreatureBehaviorContext context, GameTime time)
    {
        ...
        {
            needs.Hunger = Math.Min(1f, needs.Hunger + 0.003f * time.DeltaSeconds);
            needs.Tiredness = Math.Min(1f, needs.Tiredness + 0.002f * time.DeltaSeconds);
        }

        var xp = context.Entity.TryGetComponent<ExperienceComponent>();
        if (xp is not null)
        {
            xp.CurrentExperience += 3;
            xp.LastSource = ExperienceSource.Training;
        }
    }
}
